...
export async function setThumbnailStatusAction(formData: FormData) {
  ...
  const scope = and(eq(videos.id, parsed.data.videoId), eq(videos.status, "agendado"));
  const [video] = await db.select({ title: videos.title }).from(videos).where(scope).limit(1);
  if (!video) return;
