...
const sumInt = (column: Parameters<typeof sql>[1]) => sql<number>`coalesce(sum(${column}), 0)::int`;
...
export async function getMonthRetention(month: string) {
  const [row] = await db
    .select({
      weighted: sql<number>`coalesce(sum(${videoMetrics.avgViewPct} * ${videoMetrics.views}) / nullif(sum(${videoMetrics.views}), 0), 0)::float`,
    })
    .from(videoMetrics)
    .where(eq(videoMetrics.month, month));
  return row.weighted;
...
export async function getIdeaCounts() {
  const rows = await db.select({ status: ideas.status, n: sql<number>`count(*)::int` }).from(ideas).groupBy(ideas.status);
